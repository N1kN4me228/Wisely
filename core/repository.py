...
from core.activity import activity_days, best_streak, current_streak
...
class Repository:
    ...
    def get_activity_summary(self, user_id: int) -> dict:
        """Return daily activity and streak metrics for one learner."""
        conn = get_connection()
        rows = conn.execute(
            "SELECT created_at FROM attempts WHERE user_id = ? ORDER BY created_at",
            (user_id,),
        ).fetchall()
        conn.close()
        days = activity_days([row["created_at"] for row in rows])
        return {
            "days": days,
    ...
    def get_teacher_learners(self) -> list[dict]:
        """Return compact learner activity data for the teacher overview."""
        conn = get_connection()
        users = conn.execute("SELECT id, name, class_number FROM users ORDER BY id").fetchall()
        result = []
        for user in users:
            attempts = conn.execute(
                "SELECT created_at, score FROM attempts WHERE user_id = ? ORDER BY created_at",
                (user["id"],),
            ).fetchall()
            days = activity_days([row["created_at"] for row in attempts])
            average = round(sum(row["score"] for row in attempts) / len(attempts)) if attempts else 0
            result.append({
                "id": user["id"],
                "name": user["name"],
